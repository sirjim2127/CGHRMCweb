    ...
    <div className="w-screen h-screen flex flex-col overflow-y-auto">
      ...
      <div className="min-h-32 h-32 w-full bg-blue-500 flex p-4">
        ...
        <div className="h-full flex-col justify-center px-4 hidden md:flex overflow-y-auto">
          ...
          <h1 className="text-3xl font-bold text-[#FFDE00]">
      ...
      <div className="flex items-center px-2 min-h-20 py-4 bg-white shadow-md sticky top-0 z-10">
        <h1 className="text-xl font-bold text-gray-500 block md:hidden">
          Philippine Coast Guard
        </h1>
        <ul className="hidden h-full flex-1 justify-end items-center lg:flex">
          {MenuItems.map((item, index) => (
            <Link key={index} to={item.path}>
              <li className="text-xl text-gray-500 font-semibold mr-4 cursor-pointer hover:text-blue-500">
                {item.label}
              </li>
            </Link>
          ))}
